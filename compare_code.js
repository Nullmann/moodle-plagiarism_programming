...
/**
 * This is the javascript code of the compare code page (to compare the
 * similarity of 2 students' code) It include ajax calls to mark a student as
 * suspicious and cross similarity of one student with another, as well as
 * showing the similarity history chart
 * @package plagiarism
...
M.plagiarism_programming.compare_code = {
    ...
    init : function (Y, info, name_table, summary_table, result_table, anchor) {
        ...
        if (anchor >= 0) {
            this.move_frame('programming_result_comparison_bottom_left', 'sim_'
                    + anchor);
            this.move_frame('programming_result_comparison_bottom_right',
                    'sim_' + anchor);
        }
    ...
    init_compare_code : function (Y) {
        ...
        if (this.name_table[this.student1]) {
            div.append('<br/>');
            var checkbox1 = this
                    .create_checkbox_for_turning_on_cross_similarity('programming_result_comparison_bottom_left');
        ...
        if (this.name_table[this.student2]) {
            div.append('<br/>');
            var checkbox2 = this
                    .create_checkbox_for_turning_on_cross_similarity('programming_result_comparison_bottom_right');
    ...
    init_history_chart : function (Y) {
        ...
        var link = Y.Node.create('<a id="show_history_link" href="#">'
                + history + '</a>');
    ...
    change_image : function (action) {
        ...
        img.set('src', src);
        img.setStyle('display', (action == null || action == '') ? 'none'
                : 'inline')
    },
    ...
    unselect_blocks : function (div_class) {
        M.plagiarism_programming.compare_code.Y.all(
                'div.' + div_class + ' font.block_selected').each(
                function (similar_block) {
    ...
    show_similarity_other_students : function (div_class) {
        M.plagiarism_programming.compare_code.Y.all(
                'div.' + div_class + ' span').each(
                function (span) {
                    if (span.getAttribute('type') == 'end') {
                        var img = M.plagiarism_programming.compare_code
                                .create_image_to_show_similarity_with_others();
                        span.append(img);
                    }
                });
    },
    ...
    hide_similarity_with_other_students : function (div_class) {
        M.plagiarism_programming.compare_code.Y.all(
                'div.' + div_class + ' .student_similarity_view_img').each(
                function (img) {
                    img.remove(true);
                });
    },
    ...
    create_image_to_show_similarity_with_others : function () {
        var Y = M.plagiarism_programming.compare_code.Y;
        var img = Y.Node
                .create('<img src="pix/list_student.png" class="student_similarity_view_img" />');
    ...
    uncolor_portion : function (end_span) {
        ...
        while (!this.is_end_node(prev_node, sid)) {
            if (prev_node.nodeType == 3
                    && prev_node.parentNode.getAttribute('class') == 'colored') {
    ...
    is_end_node : function (node, sid) {
        return node.nodeType == 1 && node.tagName == 'SPAN'
                && node.getAttribute('type') == 'begin'
                && node.getAttribute('sid') == sid;
    },
    ...
    show_menu : function (span) {
        ...
        while (className != 'programming_result_comparison_bottom_left'
                && className != 'programming_result_comparison_bottom_right') {
        ...
        var menu_list = Y.Node
                .create('<ul class="programming_student_similarity_menu"/>');
        for (var i = 0; i < sids.length; i++) {
            var name = this.name_table[sids[i]];
            var std1 = Math.max(this_student, sids[i]);
            var std2 = Math.min(this_student, sids[i]);
            if (std1 && std2) {
                var href = 'view_compare.php?id='
                        + this.result_table[std1][std2] + '&anchor='
                        + anchors[i];
                menu_list.append('<li><a href="' + href + '">' + name
                        + '</a></li>');
            }
        }
        menu.set('bodyContent', menu_list);
        menu.align(span.firstChild, [ Y.WidgetPositionAlign.TL,
    ...
    create_checkbox_for_turning_on_cross_similarity : function (className) {
        ...
        checkbox.on('click', function (e) {
            if (checkbox.get('checked')) {
                M.plagiarism_programming.compare_code
                        .show_similarity_other_students(className);
            } else {
                M.plagiarism_programming.compare_code
                        .hide_similarity_with_other_students(className);
            }
        });
        return checkbox;
    },
    ...
    create_chart : function () {
        ...
        this.history_overlay.align(Y.one('#show_history_link'), [
                Y.WidgetPositionAlign.TR, Y.WidgetPositionAlign.BR ]);

        Y.io('mark_result.php?task=get_history&id='
                + M.plagiarism_programming.compare_code.id, {
                    on : {
    ...
    load_overlay : function (id, o) {
        var Y = M.plagiarism_programming.compare_code.Y;
        var history = M.plagiarism_programming.compare_code.Y.JSON
                .parse(o.responseText);

        var overlay = Y.Node
                .create('<div class="programming_result_chart_overlay"></div>');
        var canvas = Y.Node
                .create('<div class="programming_result_popup_chart"></div>');
        var h_label = Y.Node.create('<label class="h_label">'
                + M.str.moodle.date + '</label>');
        ...
        for (var i in history) {
            ...
            var label = Y.Node.create('<label>' + history[i].similarity
                    + '%</label>');
            label.setStyles({
                left : left + 'px',
                bottom : (history[i].similarity / 100 * canvas_height + 5)
                        + 'px'
            });
            canvas.append(label);

            label = Y.Node
                    .create('<label>' + history[i].time_text + '</label>');
            label.setStyles({
                left : left + 'px',
                bottom : '-35px'
            })
        ...
        M.plagiarism_programming.compare_code.history_overlay.set(
                'bodyContent', overlay);
        M.plagiarism_programming.compare_code.history_overlay
    ...
    init_summary_table : function () {
        ...
        if (Y.version == '3.4.1') {
            ...
            table.render('.simiarity_table_holder');
            table.set('scrollable', (table.get('contentBox')
                    .get('clientHeight') >= table._tableNode
                    .get('scrollHeight')) ? false : 'y');
        }
